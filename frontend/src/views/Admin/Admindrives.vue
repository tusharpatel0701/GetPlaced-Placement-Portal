...
async function fetchDrives() {
  ...
  try {
    ...
    error.value = "Could not load drives. Please try again.";
  } finally {
    loading.value = false;
  }
}
...
async function handleAction(driveId, action) {
  actionLoading.value = driveId + action;
  actionError.value = "";
  try {
    const token = localStorage.getItem("token");
  ...
  <div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
      <div>
        ...
        <p class="text-muted mb-0">Review and approve placement drives posted by companies</p>
      </div>
    ...
    <div v-else-if="drives.length === 0" class="text-center py-5 text-muted">
      <p class="fs-5">No placement drives found.</p>
    </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              <tr v-for="(drive, index) in drives" :key="drive.id">
                ...
                <td class="text-muted">{{ drive.company }}</td>
                <td class="text-muted">{{ formatDate(drive.deadline) }}</td>
                <td>
                  <span :class="statusBadgeClass(drive.status)">
                ...
                <td class="text-center">
                  <div class="d-flex justify-content-center gap-2">
                    <button
                      ...
                      :disabled="drive.status === 'Approved' || actionLoading === drive.id + 'approve'"
                      @click="handleAction(drive.id, 'approve')"
                    >
                      <span v-if="actionLoading === drive.id + 'approve'" class="spinner-border spinner-border-sm"></span>
                      <span v-else>Approve</span>
                    </button>
                    <button
                      class="btn btn-sm btn-danger"
                      :disabled="drive.status === 'Rejected' || actionLoading === drive.id + 'reject'"
                      @click="handleAction(drive.id, 'reject')"
                    >
                      <span v-if="actionLoading === drive.id + 'reject'" class="spinner-border spinner-border-sm"></span>
