...
def get_student_profile(student_id):
    key = f"student_profile_{student_id}"
    data = cache.get(key)
    if data is None:
...
def get_all_approved_drives():
    data = cache.get("student_approved_drives")
    ...
    if data is None:
        ...
        cache.set("student_approved_drives", data, timeout=300) # 5 mins
    return data, 200
...
def apply_for_drive(drive_id, data):
    ...
    db.session.commit()
    cache.delete(f"student_applications_{student_id}")         
...
def get_student_applications(student_id):
    key = f"student_applications_{student_id}"
    data = cache.get(key)
    if data is None:
        student = Student.query.get(student_id)
        if not student:
